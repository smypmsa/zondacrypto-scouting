...
- **42 inventoried wallets and clusters** across Bitcoin, Ethereum, and Polygon. See [`wallets/README.md`](wallets/README.md) for the per-row index. The most recent additions are the 2026-05-23 cohort of eight Ethereum deposit-receipt addresses surfaced by a structural predictor (`out_to_z5_pct = 1.0` AND `distinct_cex_in ≥ 3` AND `in_to_z5 ≥ $5M`) and confirmed individually by BlockSec MetaSleuth — each sweeps 100% of outbound flow to Zonda 5 and aggregates inbound deposits from many hildobby-tagged centralised-exchange hot wallets.
