...
1. **Transaction relation.** All three addresses receive material ETH-equivalent token flow directly from the confirmed Zonda 5 multichain key (`0x6edf968d…`) on Polygon. The Zonda 5 key itself carries a primary BlockSec MetaSleuth attribution as `Zonda (BitBay)` on Polygon (chain_id 137) captured 2026-05-21 — receipt at [`../../sources/blocksec/labels-2026-05-21.json`](../../sources/blocksec/labels-2026-05-21.json), key `0x6edf968da408a9640b8865826429a977a11c5048@137`. The two sibling addresses sit at the very top of Zonda 5's outbound counterparty distribution on Polygon (#1 and ranked) — a position only a same-operator address could plausibly occupy.
