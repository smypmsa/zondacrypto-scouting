...
- Native txs: first 2017-12-11 (block 4714328); most recent 2026-03-24 (block 24727471); ≥ 10,000 events (the indexer’s per-page cap — the true count is at least that; the full count is recovered from Dune in the Flow profile section below).
...
**Institutional multi-hop exposure (non-CEX).** Two hops downstream of Zonda 1 on its USDC OUT side sits the address `0xc3acf3b96e46aa35dbd2aa3bd12d23c11295e774`, attributed by BlockSec MetaSleuth on 2026-05-22 as `Maker: Rwa007 A Coinbase Custody` — the Coinbase Custody settlement address for MakerDAO's RWA-007 real-world-asset vault. The on-chain path is Zonda 1 → relay address `0x1990abd6…244181` → MM/OTC hub address `0x5ee84d30…6916b` → MakerDAO RWA-007 Coinbase Custody; the relay and the hub do not themselves carry a BlockSec primary attribution at the 2026-05-22 index date and remain in this page's Unattributed-counterparties section. The bilateral aggregate from the MM/OTC hub to the MakerDAO RWA-007 address is $552M USDC across 12 lifetime events. The on-chain graph establishes the multi-hop path but does not establish that any specific Zonda customer USDC dollar reached the MakerDAO vault. See [`observations/institutional-counterparties.md`](../../observations/institutional-counterparties.md) for the BlockSec receipt details and methodology.
