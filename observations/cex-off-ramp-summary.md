# Cross-wallet CEX off-ramp summary (Ethereum, Dune-aggregated 2026-05-13; BlockSec extensions 2026-05-21, 2026-05-22)

Sum of outbound USD-equivalent value (at transaction time) from each ZondaCrypto inventory wallet to centralised-exchange addresses carrying a primary public attribution. Two attribution sources are used in this summary: (1) Etherscan Public Name Tags verified by fresh HTML fetch on 2026-05-13 for every row in the Etherscan-tagged tables below; and (2) BlockSec MetaSleuth address-label API v3 responses captured 2026-05-21 and 2026-05-22 for further addresses that carry no Etherscan public name tag. Each row remains directly re-checkable: the Etherscan-tagged rows against the live block-explorer page, the BlockSec-resolved rows against the mirrored JSON receipts.

The 2026-05-22 BlockSec batch resolved five further addresses on the Zonda 5 outbound side: three Binance-customer deposit addresses (`0xcbc537685c…4680`, `0x558b6d62…07c9`, `0x7ff423f8…4c76`) and two Kraken-customer deposit addresses (`0x19f0d946…1cc7`, `0x3372e8c4…a7a1`). Across the union of all attribution sources, **$211,477,461 in Etherscan-tagged outbound** plus **$61,056,743 in BlockSec-resolved outbound** is now CEX-attributable from the inventoried Ethereum wallets — a combined **~$272,534,204** of outbound flow lands on addresses with a primary CEX attribution.
...
**Confidence.** CONFIRMED on totals: every cell is a Dune-aggregation sum filtered to addresses with a primary CEX attribution (Etherscan public name tag verified by fresh HTML fetch 2026-05-13, or BlockSec MetaSleuth `main_entity` returned 2026-05-21 / 2026-05-22). PARTIAL on rows where the source-wallet attribution itself is at PARTIAL tier (the BitBay-era wallets — flagged below).

## Per-CEX × per-source-wallet outbound (USD)

Rows tagged **(BlockSec YYYY-MM-DD)** are attributed by BlockSec MetaSleuth API v3 against an address that carries no Etherscan public name tag; receipts mirrored at [`../sources/blocksec/labels-2026-05-21.json`](../sources/blocksec/labels-2026-05-21.json) and [`../sources/blocksec/labels-2026-05-22.json`](../sources/blocksec/labels-2026-05-22.json). All other rows are Etherscan-tagged (HTML fetch 2026-05-13). Rows attributing to a CEX *customer's* deposit address (`name_tag` ends in `: Deposit Address`) are flagged as **customer-aggregator** class — structurally these are individual CEX customers' deposit accounts into which Zonda customers receive withdrawals, which the CEX then internally sweeps to its hot wallets; the identity of the CEX customer behind each deposit address is out of scope of this repository.
...
| **Binance (customer-aggregator)** | **BlockSec MetaSleuth 2026-05-22** | **Zonda 5 `0x6edf968d…5048`** | **`0xcbc537685c…4680`** (`Binance: Deposit Address`) | **$15,823,359** | **61** | **1** |
| **Binance (customer-aggregator)** | **BlockSec MetaSleuth 2026-05-22** | **Zonda 5 `0x6edf968d…5048`** | **`0x558b6d62…07c9`** (`Binance: Deposit Address`) | **$10,942,800** | **64** | **1** |
| **Binance (customer-aggregator)** | **BlockSec MetaSleuth 2026-05-22** | **Zonda 5 `0x6edf968d…5048`** | **`0x7ff423f8…4c76`** (`Binance: Deposit Address`) | **$7,897,901** | **54** | **1** |
...
| **Kraken (customer-aggregator)** | **BlockSec MetaSleuth 2026-05-22** | **Zonda 5 `0x6edf968d…5048`** | **`0x19f0d946…1cc7`** (`Kraken: Deposit Address`) | **$9,182,985** | **9** | **1** |
| **Kraken (customer-aggregator)** | **BlockSec MetaSleuth 2026-05-22** | **Zonda 5 `0x6edf968d…5048`** | **`0x3372e8c4…a7a1`** (`Kraken: Deposit Address`) | **$6,809,697** | **4** | **1** |
...
The five Z5-source customer-aggregator rows do not overlap with the Etherscan-tagged Binance / Gate Z5 rows above: every Etherscan-tagged row's counterparty carries a public `Binance Deposit: …` / `Gate Deposit: …` name tag on Etherscan, whereas all five customer-aggregator counterparties carried no Etherscan public name tag at the 2026-05-13 fetch and were only resolved by the 2026-05-22 BlockSec MetaSleuth probe. They sit in the Zonda 5 wallet page's `Unattributed counterparties` section under the prior framing and are surfaced into this attributed table by the new BlockSec attribution.

## Per-CEX totals (across all source wallets)

| CEX brand | Etherscan-tagged outbound USD | BlockSec-resolved outbound USD | Combined outbound USD |
|--|--|--|--|
| Binance | $203,073,425 | $34,664,060 | $237,737,485 |
| Gate | $8,400,941 | — | $8,400,941 |
| Bybit | $3,096 | — | $3,096 |
| Kraken | — | $15,992,682 | $15,992,682 |
| WhiteBIT | — | $10,400,000 | $10,400,000 |
| **All CEX brands combined** | **$211,477,461** | **$61,056,742** | **$272,534,203** |
