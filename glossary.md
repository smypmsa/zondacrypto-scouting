...
- **Vanity-suffix family** — A set of addresses sharing a common hex prefix or suffix produced by vanity-address-generation software. A shared vanity pattern does **not** imply a common operator: address-poisoners deliberately mimic legitimate addresses' vanity patterns to confuse victims, so a vanity-suffix family can mix genuine operator addresses with adversarial lookalikes. Cluster boundaries within a vanity family are drawn by independent evidence — on-chain gas-funder fingerprint (see [methodology § Per-address phishing classification](methodology.md#per-address-phishing-classification--etherscan-v2-free-tier-method)) or BlockSec entity attribution — not by the shared suffix alone.
...
- **Multichain EVM key reuse** — The same Ethereum-format address (20-byte EVM key) is active on two or more EVM-compatible chains (Ethereum, Polygon, Arbitrum, Optimism, Avalanche-C, BSC, Base, zkSync Era, …). Because EVM-compatible chains share the same address derivation, the same private key controls the address on every EVM chain it is deployed to. Used as cross-chain attribution evidence; **the same BlockSec entity label returned for the same EVM key across multiple chains is the strongest form of confirmation** (see [methodology § Multichain EVM-key-reuse customer-aggregator detection](methodology.md#multichain-evm-key-reuse-customer-aggregator-detection)).
...
- **Customer aggregator** (e.g. **Kraken customer aggregator**, **Binance customer aggregator**) — An externally-owned address controlled by a single customer of a centralised exchange, used to collect funds from many sources before forwarding to the exchange's hot wallets. Distinct from an exchange-*owned* deposit address (assigned by the exchange to an individual customer for a single chain) and from an exchange hot wallet. Detection signature: BlockSec MetaSleuth returns a `<Exchange>: Deposit Address` label on the EOA, and **the same label appears for the same EVM key on multiple EVM chains** — indicating one customer registered the same key as their deposit address on each chain, rather than the exchange operating that EVM key as its own infrastructure. See [methodology § Multichain EVM-key-reuse customer-aggregator detection](methodology.md#multichain-evm-key-reuse-customer-aggregator-detection).
- **Dust-poisoner** — An externally-owned address whose entire outbound activity consists of zero-value or sub-cent transfers to other addresses, with the goal of populating victims' transaction histories with addresses that visually resemble their legitimate counterparties. Outbound transfers typically use one of: dust amounts of canonical USDC / USDT (raw value `10` = $0.00001 at six decimals); fake-token Transfer events with lookalike `symbol` fields (Cyrillic or Greek substitutions, or ASCII spoofs like `U5DT` / `U5DC` / `DAl`); or a few gwei of native ETH. The address typically carries an Etherscan `Fake_Phishing*` public name tag. See [methodology § Per-address phishing classification](methodology.md#per-address-phishing-classification--etherscan-v2-free-tier-method) for the OUT-side / IN-side test that distinguishes a dust-poisoner from a poisoning *victim* who has been tagged by Etherscan's crowd-sourced flag.
